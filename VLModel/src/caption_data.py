...
# vsd_dir = dataset_dir.joinpath('sp3000')
# vsd_img_dir = sp_dir.joinpath('images/')
# sp_feature_dir = sp_dir.joinpath('features')
# coco_dir = dataset_dir.joinpath('COCO')
img_dir = dataset_dir.joinpath('images')
...
class VSDCaptionFineTuneDataset(Dataset):
    def __init__(self, split='train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train'):
        ...
        if self.args.max_n_boxes == 36:
            self.source_to_h5 = dataset_dir.joinpath('features').joinpath('vrd_boxes36.h5')
...
def get_vsd_loader(args, split='train', mode='train',
    ...
    dataset = VSDCaptionFineTuneDataset(
        split,
        # raw_dataset=_dset,
        rank=gpu,
        topk=topk,
...
class COCOCaptionEvaluator:
    def __init__(self):
        ...
        self.evaluator = language_evaluation.CocoEvaluator(verbose=False, coco_types=["BLEU", "SPICE"])
